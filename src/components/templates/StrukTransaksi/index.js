    ...
    <div style={struk_transaksi}>
      ...
      <table style={tableWrapper}>
        <tr style={{ columnSpan: 4 }}>
          <td style={{ textAlign: 'left' }}>
            {validateCookiey("init") && JSON.parse(getCookie("init")).trans_id ?  '' : `No Meja : ${data.nomor_name}`}
      ...
      <table style={tableWrapper}>
        <tr style={{ columnSpan: 2 }}>
          <td style={{ textAlign: 'left' }}>
            {`Kasir: ${data.casheer_name}`}
          </td>
            <td style={{ textAlign: 'right' }}>{`Self Order${data.consume_type === 'dine_in' ? '/Dine In' : validateCookiey("init") && JSON.parse(getCookie("init")).trans_id ? '' : '/Take Away'}`}</td>
        </tr>
        <tr style={{ columnSpan: 2 }}>
          <td style={{ textAlign: 'left' }}>
            {`No Transaksi: ${data.order_id}`}
          </td>
          <td style={{ textAlign: 'right' }}>{moment(data.created_at).format('DD MMMM YYYY HH:mm')}</td>
        </tr>
      </table>
      ...
      <table style={tableWrapper}>
        <tr style={{ columnSpan: 6, width: '100%' }}>
          ...
          <td style={{ textAlign: 'left', width: '70%' }}>Transaksi Berhasil!</td>
          <td />
          <td />
          <td />
          <td />
      ...
      <table style={tableWrapper}>
        ...
        {data && data.detil && data.detil.map((item) => (
          <>
            <tr>
              <td style={{ textAlign: 'left' }}>
                {item.product_name}
              </td>
              <td style={{ width: 'fit-content', textAlign: 'center' }}>{`${item.product_qty}x `}</td>
              <td style={{ textAlign: 'left' }}>{convertCurrency(item.product_base_price)}</td>
              <td style={{ textAlign: 'right' }}>{convertCurrency(item.product_total_price)}</td>
            </tr>
            {item.customize.length > 0 && item.customize.map((item) => (
              <tr>
                <td style={{ textAlign: 'left' }}>
                  {`${item.customize_name}: ${item.pilihan_name}`}
                </td>
                <td />
                <td style={{ textAlign: 'left' }}>{convertCurrency(item.pilihan_price)}</td>
                <td />
              </tr>
            ))}
            {item.product_note && (
      ...
      <table style={tableWrapper}>
        ...
        {data && data.addon_price && data.addon_price.map((item) => (
          <tr style={{ columnSpan: 2 }}>
            <td style={{ textAlign: 'left' }}>
              {item.name}
            </td>
    ...
    Barang yang sudah dibeli tidak dapat ditukar/dikembalikan.
      ...
      <table style={tableWrapper}>
        <tr style={{ columnSpan: 6, width: '100%' }}>
          ...
          <td style={{ textAlign: 'left', width: '70%' }}>{data.instagram || '-'}</td>
          <td />
          <td />
          <td />
          <td />
      ...
      <table style={tableWrapper}>
        <tr style={{ columnSpan: 6, width: '100%' }}>
          ...
          <td style={{ textAlign: 'left', width: '70%' }}>{data.facebook || '-'}</td>
          <td />
          <td />
          <td />
          <td />
      ...
      <table style={tableWrapper}>
        <tr style={{ columnSpan: 6, width: '100%' }}>
          ...
          <td style={{ textAlign: 'left', width: '70%' }}>{data.website || '-'}</td>
          <td />
          <td />
          <td />
          <td />
